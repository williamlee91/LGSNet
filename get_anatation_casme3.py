...
def process_one_video_reduce(fpath, svideopath, sss, before, after, start=0, end=0):
    # the basenames of all pics in one video
    all_pic = os.listdir(fpath) 
    print(fpath)
    all_pic = [int(str(i).split('.')[0]) for i in all_pic]
    ...
    all_pic_diff = numpy.diff(numpy.array(all_pic))
    oom_index = numpy.argwhere(all_pic_diff >= 2).squeeze(-1)
    miss_num = 0 # the number of plunging pics
    save_label = True
    # record the number of shifted frames 
    if oom_index.any():
        for k in oom_index.tolist():
            # Count the number of missing frames
            if all_pic[k+1] <=start:
                miss_num = all_pic[k+1] - all_pic[k] + 1
            elif start <= all_pic[k]< end or start< all_pic[k+1] <= end: 
                print("error gt!!", fpath, start, end)
                save_label = False
            else:
                pass
    ...
    # copy remaining pics into the new data folder
    if len(os.listdir(svideopath)) < len(all_pic):
        for p in range(len(all_pic)):
            pic_tmp = cv2.imread(os.path.join(fpath, str(all_pic[p])+'.jpg'))
            cv2.imwrite(os.path.join(svideopath, str(p + 1)+'.jpg'), pic_tmp)

    return sss, before, after, firstname, miss_num, save_label


def main(e_path, v_path, l_path, extract_full_data, plunging, delete_long):
    ...
    for i in range(len(df_sheet['Subject'].values)):
        ...
        if extract_full_data and plunging:
            basepath = os.path.join(path_full.split('/part_A')[0], 'new_data_full')
        elif not extract_full_data and not plunging:
            basepath = os.path.join(path_full.split('/part_A')[0], 'new_data_train')
        elif extract_full_data and not plunging:
            basepath = os.path.join(path_full.split('/part_A')[0], 'new_data_full_reduce')
        ...
        if plunging:
            ...
            sss, before, after, firstname, miss_num, label_saved = process_one_video_reduce(path_full, video_save_path, sss, before, after, startframe, endframe)
        ...
        if label_saved:
            real_start = real_start - miss_num
            real_end = real_end - miss_num
            
        if real_end <= count_pic and delete_long:
            if ((label == 'Micro-expression' and real_end - real_start <= 15) or (label == 'Macro-expression' and real_end - real_start <= 120)) and label_saved:
        ...
        if extract_full_data: 
            for vn in unlabeled_videoname:
                ...
                if plunging:
                    sss, before, after, _ = process_one_video(vn_all, vsp_all, sss, before, after)
                else:
                    sss, before, after, _, _, _ = process_one_video_reduce(vn_all, vsp_all, sss, before, after)
    ...
    if extract_full_data and delete_long and not plunging:
        df.to_csv('cas3_annotation_full_me_reduce.csv', encoding='utf-8', index=False)
    elif extract_full_data and delete_long and plunging:
        df.to_csv('cas3_annotation_full_me.csv', encoding='utf-8', index=False)
    elif extract_full_data and not delete_long and plunging:
        df.to_csv('cas3_annotation_full.csv', encoding='utf-8', index=False)
    elif not extract_full_data and not delete_long and plunging:
        df.to_csv('cas3_annotation_full.csv', encoding='utf-8', index=False)
    else:
        pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('-len_path', type=str, default='./cas(me)3_len.txt')
    ...
    parser.add_argument('-plunge', action='store_true', default=False, help='if true, delete these gts which intersect with missing frames')
    ...
    main(excel_path, video_files, len_path, extract_full_data, plunge, delete_lme)
